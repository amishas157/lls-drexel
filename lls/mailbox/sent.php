...
<?php
	...
	global $database;
	$sql = "select COUNT(*) FROM messages where sender_id=".$_SESSION['user_id']." and is_deleted='false' order by id desc" ;
	...
	$sql = "select * from messages where sender_id=".$_SESSION['user_id']." and is_deleted='false' order by id desc " ;
	$sql .= "LIMIT {$per_page} ";
	$sql .="OFFSET {$pagination->offset()}";
	
	$sent_message_ids = Message::find_by_sql( $sql);
...
<head>
	...
	<title>LLSCT | Mailbox  |Sent</title>
	...
	<div class="mail-body">
		...
		<table class="table mail-table">
							...
							<?php 
							   if($pagination->total_pages() >1) {
							       if($pagination->has_prev_page()) {
								     ?>
								<a href="../mailbox/sent.php?page=<?php echo$pagination->prev_page()?>" class="btn btn-sm btn-white"><i class="entypo-left-open"></i></a>
								<?php }
								}
								?>
							   ...
							   if($pagination->total_pages() >1) {
							       if($pagination->has_next_page()) {
								     ?>
								<a href="../mailbox/sent.php?page=<?php echo$pagination->next_page()?>" class="btn btn-sm btn-white"><i class="entypo-right-open"></i></a>
			...
			<tbody>
				...
				<?php 
					foreach( $sent_message_ids as $msg )
					{
						$message = Message::find_by_id( $msg->id );
						?>	 
						...
						<tr class="read"><!-- new email class: unread -->
							...
							<td class="col-name">
								<a href="mailbox-message?id=<?php echo $message->id; ?>&type=sent" class="col-name"> 
									<?php echo $message->subject; ?> 
								</a>
							</td>
							<td class="col-subject">
								<a href="mailbox-message?id=<?php echo $message->id; ?>&type=sent">
