	...
	<div class="mail-body">
		...
		<table class="table mail-table">
			...
			<thead>
				<tr>
					...
					<th>
						...
						<button type="submit" name="action" value="recover" class="btn btn-green btn-icon icon-left">
							Recover
							...
							<div class="btn-group">
							   ...
							   if($pagination->total_pages() >1) {
							       if($pagination->has_prev_page()) {
								     ?>
								<a href="../mailbox/trash.php?page=<?php echo$pagination->prev_page()?>" class="btn btn-sm btn-white"><i class="entypo-left-open"></i></a>
								<?php }
								}
								?>
							   ...
							   if($pagination->total_pages() >1) {
							       if($pagination->has_next_page()) {
								     ?>
								<a href="../mailbox/trash.php?page=<?php echo$pagination->next_page()?>" class="btn btn-sm btn-white"><i class="entypo-right-open"></i></a>
			...
			<tbody>
				...
				<?php 
					foreach( $trash_message_ids as $msg )
					{
						...
						<tr class="read"><!-- new email class: unread -->
							...
							<td class="col-name">
								<a href="mailbox-message?id=<?php echo $message->id; ?>&type=trash" class="col-name"> 
									<?php echo $message->subject; ?> 
								</a>
							</td>
							<td class="col-subject">
								<a href="mailbox-message?id=<?php echo $message->id; ?>&type=trash">
									<?php
									if( $msg->type == 'inbox' )
										echo '<span class="label label-warning">Inbox</span>';
									else
