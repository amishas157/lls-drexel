...
<h2>Choose a survey to respond</h2>
...
$survey_array=SurveyData::find_all();
$empty=0;
foreach($question_array as $question_obj)
{
 
$question = SurveyData::find_by_question_response($question_obj->question_id, $_SESSION['user_id']);
$count=0;
foreach($survey_array as $survey)
{
  if($survey->question_id==$question_obj->question_id)
   {
    $count++;
   }
}
$percent=($count/7)*100;
	...
	if(!$question&&($question_obj->is_enabled=="true"))
        {$empty++;
          ?>
	...
	<div class="col-sm-3">	
		<div class="tile-progress tile-primary">
			...
			<div class="tile-progressbar">
				<span data-fill="<?php echo number_format($percent , 2 , '.' , '')  ?>"></span>
			</div>
			...
			<div class="tile-footer">
				...
				<span>
					<button 
						type="button" 
						class="btn btn-info" 
						onClick="window.location.assign('respond-survey.php?id=<?php echo $question_obj->question_id?>')" 
					>
						Respond To Survey
				</button>
				</span>
...
{
 ?>
<h3> Sorry! You do not have a survey to respond </h3>
<?php
 }
?>
